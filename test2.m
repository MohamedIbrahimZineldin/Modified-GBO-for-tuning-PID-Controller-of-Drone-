...
% contro=5;
%-----------------------------------------------------------------------------
[lb,ub,dim,fobj]=Get_Functions_details('F2'); % phi PID CONTROL
%% Initialization
...
Xnew=X;
for i=1:nP
    Cost(i) = fobj(Xnew(i,:));      % Calculate the Value of Objective Function
...
%             aa= rand-rand^(   it/MaxIt     );                                       
            if u1<0.5
%                 Xnew = Xnew + f1.*(u1.*Best_X-u2.*Xp)+f2.*ro.*(u3.*(X2-X1)+u2.*(X(r1,:)-X(r2,:)))/2;     
                Xnew = Xnew + cos(f1).*(u1.*Best_X-u2.*Xp)+f2.*ro.*(u3.*(X2-X1)+u2.*(X(r1,:)-X(r2,:)))/2;     %Modified
            else
%                 Xnew = Best_X + f1.*(u1.*Best_X-u2.*Xp)+f2.*ro.*(u3.*(X2-X1)+u2.*(X(r1,:)-X(r2,:)))/2;   
                Xnew = Best_X + sin(f1).*(u1.*Best_X-u2.*Xp)+f2.*ro.*(u3.*(X2-X1)+u2.*(X(r1,:)-X(r2,:)))/2;   %Modified
            end
        end
