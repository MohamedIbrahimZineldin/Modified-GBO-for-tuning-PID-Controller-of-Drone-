...
function [Best_Cost,Best_X,Convergence_curve]=GBOSCA(nP,MaxIt,lb,ub,dim,fobj)
%% Initialization
...
%                 Xnew = Xnew + f1.*(u1.*Best_X-u2.*Xp)+f2.*ro.*(u3.*(X2-X1)+u2.*(X(r1,:)-X(r2,:)))/2;     
                Xnew = Xnew + cos(f1).*(u1.*Best_X-u2.*Xp)+f2.*ro.*(u3.*(X2-X1)+u2.*(X(r1,:)-X(r2,:)))/2;     %ÊÚÏíá ãÍãÏ ÇÈÑÇåíã
            else
%                 Xnew = Best_X + f1.*(u1.*Best_X-u2.*Xp)+f2.*ro.*(u3.*(X2-X1)+u2.*(X(r1,:)-X(r2,:)))/2;   
                Xnew = Best_X + sin(f1).*(u1.*Best_X-u2.*Xp)+f2.*ro.*(u3.*(X2-X1)+u2.*(X(r1,:)-X(r2,:)))/2;   
            end
        end
